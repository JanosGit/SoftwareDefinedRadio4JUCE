...
namespace ntlab
{
#if JUCE_MAC
    /**
     * This header is a direct copy of the juce::DynamicLibrary interface and can therefore be used as drop-in
     * replacement for the JUCE version. The juce::DynamicLibrary implementation had some bugs on Mac OS, so this one
     * uses NSTask to launch the ChildProcess under the hood. On non-Mac systems it just is an alias to the JUCE
     * implementation.
     */
    class ChildProcess
    {
    public:
        ...
        */
        bool start (const juce::String& command, int streamFlags = wantStdOut | wantStdErr);
        ...
        /** Attempts to launch a child process command.
            ...
            streams should be read and returned by readProcessOutput().
        */
        bool start (const juce::StringArray& arguments, int streamFlags = wantStdOut | wantStdErr);
...
#else
    using ChildProcess = juce::ChildProcess;
#endif
